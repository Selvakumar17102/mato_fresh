<?php
    ...
    if(!empty($data->orderid))
    {
        ...
        if($result->num_rows > 0)
        {
            ...
            $tem["Veggis"]["id"] = $row["sno"];
            $tem["Veggis"]["orderid"] = $oid;
            $tem["Veggis"]["booked_date"] = date('d-m-Y', strtotime($row["booking_date"]));
            $tem["Veggis"]["booked_time"] = $row["booking_time"];
            $tem["Veggis"]["delivery_slot"] = $row["delivery_slot"];
            $tem["Veggis"]["order_status"] = $order_status;
            $tem["Veggis"]["total_amount"] = $row["overall_total_amount"];
            $tem["Veggis"]["total_recipe_amount"] = $row["recipe_total_amount"];
            $tem["Veggis"]["packing_charge"] = $row["packing_charge"];
            $tem["Veggis"]["delivery_charges"] = $row["delivery_charge"];
            $tem["Veggis"]["mode_of_payment"] = $row["delivery_type"];
            $tem["Veggis"]["coupon_applied"] = $row["coupon"];
            $tem["Veggis"]["recipie_details"] = $r;
            $tem["Veggis"]["user_details"] = $d;
            $tem["Veggis"]["latitude"] = $row["lati"];
            $tem["Veggis"]["longitude"] = $row["longi"];
            $tem["Veggis"]["coupon_applied"] = $row["coupon"];
            $tem["Veggis"]["redeem_point_used"] = $row["redeem_point"];
