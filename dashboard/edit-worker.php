<?php
	...
	if(isset($_POST["add"]))
	{
        ...
        $branch = $_POST["branch"];
        $image = $_POST["image"];
        $citymain = $_POST["citymain"];

        $sql = "SELECT * FROM login WHERE username='$user' AND phone='$phone' AND id!='$id'";
        $result = $conn->query($sql);
        ...
        else
        {
            if($repass == $pass)
            {
                $sql1 = "UPDATE login SET username='$user',password='$pass',phone='$phone' WHERE id='$id'";
                if($conn->query($sql1) === TRUE)
                {
                    $sql3 = "UPDATE worker SET name='$name',email='$email',addr='$addr',image='$image',zone='$branch' WHERE lid='$id'";
                    if($conn->query($sql3) === TRUE)
                    {
						...
						if(!$resumename1=="")
						{
							...
							$resumename = "Images/Worker".$id.".".$resumetype;
							...
							if(in_array($resumetype, $allowTypes))
							{
								...
								{
									$sql1 = "UPDATE worker SET image='$resumename' WHERE lid='$id'";
									if($conn->query($sql1) === TRUE)
									{
										header("Location: add-worker.php?msg=Delivery Partner Updated!");
									}
								}
							}
						}
...
<head>
	...
	<!-- DESCRIPTION -->
	<meta name="description" content="Edit Delivery Partner | Mato Fresh" />
	
	<!-- OG -->
	<meta property="og:title" content="Edit Delivery Partner | Mato Fresh" />
	<meta property="og:description" content="Edit Delivery Partner | Mato Fresh" />
	...
	<!-- PAGE TITLE HERE ============================================= -->
	<title>Edit Delivery Partner | Mato Fresh</title>
							...
							<form class="edit-profile" method="post" enctype="multipart/form-data">
								<div class="">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-6">
                                            <input type="text" name="name" class="form-control" placeholder="Delivery Partner Name - English" required value="<?php echo $row1["name"] ?>">
                                        </div>
									...
									<div class="form-group row">
										...
										<div class="col-sm-6">
											<input type="text" name="username" class="form-control" placeholder="User Name" onkeyup="getuser(this.value)" required value="<?php echo $row["username"] ?>">
											<div style="color: red" id="user"></div>
										</div>

                                        <div class="col-sm-6">
											<input type="text" name="phone" class="form-control" placeholder="Phone Number" onkeypress="return IsAlphaNumeric(event);" ondrop="return false;" onpaste="return false;" onkeyup="getphone(this.value)" required value="<?php echo $row["phone"] ?>">
                                    ...
                                    <div class="form-group row">
										...
										<div class="col-sm1-1">
											<a class="imga" title="View Image" target="_blank" href="<?php echo $row1["image"] ?>"><img src="assets/images/icons/banner.png"></a>
										</div>
                                        <div class="col-sm-5">
									...
									<div class="form-group row">
										...
										<div class="col-sm-6">
                                            <select name="branch" class="form-control" required>
												...
												<?php
													$sql4 = "SELECT * FROM zone ORDER BY name ASC";
													$result4 = $conn->query($sql4);
													while($row4 = $result4->fetch_assoc())
													{
														$s = "";
														if($row1["zone"] == $row4["id"])
														{
															$s = "Selected";
														}
												?>
														<option <?php echo $s ?> value="<?php echo $row4["id"] ?>"><?php echo $row4["name"] ?></option>
												<?php
													}
												?>
											</select>
