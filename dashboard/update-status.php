<?php
    ...
    if($conn->query($sql) === TRUE)
    {
		...
		$linkUrl = $orderid;
		$image = $image1;
		$notification = array('title' =>$title ,'linkUrl' =>$linkUrl , 'body' => $body, 'image' => $image, 'sound' => 'default', 'badge' => '1', 'status'=> $s, 'type'=>'1');
			...
			if($result3->num_rows > 0)
			{
				...
				foreach($nullarray as $x => $x_value)
				{
					if($i < 5)
					{
						...
						$notification = array('title' =>$title,'linkUrl' =>$orderid,'body' => $body, 'image' => $img, 'sound' => 'default', 'badge' => '1', 'type'=>'1');
						$arrayToSend = array('to' => $token, 'data' => $notification,'priority'=>'high');
						$json = json_encode($arrayToSend);
						...
						else
						{
							if($m == "")
							{
								header("Location: order.php?msg=Status Updated!");
							}
							else
							{
