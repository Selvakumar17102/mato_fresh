...
<head>
	...
	<!-- PAGE TITLE HERE ============================================= -->
	<title>store inventory Reports | Mato Fresh</title>
						...
						<div class="widget-inner">
                            <div class="table-responsive">
                    	        <table id="dataTableExample1" class="table table-bordered table-striped table-hover">
									...
									<tbody>
                                        <?php
                                        $stoSql = "SELECT * FROM `login`WHERE control ='2' AND status='0'";
                                        $stoResult = $conn->query($stoSql);
                                        $count = 0;
                                        while($stoRow = $stoResult->fetch_assoc()){
                                            $count++;
                                            $stoId = $stoRow['id'];
                                            ...
                                            <tr>
                                                ...
                                                <td><center><?php echo $stoRow['username'] ?></center></td>
                                                <td>
                                                    <center>
                                                        <?php
                                                        $sql = "SELECT * FROM `store_inventory_history` a LEFT OUTER JOIN product b ON a.product_id=b.id WHERE a.store_id='$stoId' AND a.date BETWEEN '$start' AND '$date'";
...
<script>
	function boss(val)
	{
		if(val != "")
		{
			location.replace("http://mato_fresh.test/dashboard/store_inventory_report.php?id="+val);
		}
	}
	function search()
	{
		...
		else
		{
			...
			{
				location.replace("http://mato_fresh.test/dashboard/store_inventory_report.php?fdate="+fdate+"&tdate="+tdate);
			}
		}
	}
</script>
